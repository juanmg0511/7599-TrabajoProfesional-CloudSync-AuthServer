...
class AllAdminUsers(Resource):
    ...
    @helpers.log_reqId
    def get(self):
        authServer.app.logger.info(helpers.log_request_id() + 'All admin users requested.')
    ...
    def post(self):
        try:
            ...
            parser.add_argument("password", type=helpers.non_empty_string, required=True , nullable=False)
            ...
            parser.add_argument("email", type=helpers.non_empty_email, required=True , nullable=False)
            args = parser.parse_args()
        except:
            UserResponsePost = {
                "code": -1,
        ...
        authServer.app.logger.info(helpers.log_request_id() + "New adminuser '" + args["username"] + "' requested.")
        ...
        if ((existingUser is not None) or (existingAdminUser is not None)):
            UserResponsePost = {
                "code": -2,
                "message": "Bad request. Admin user '" + args["username"] + "' already exists.",
                "data": None
            }
            return helpers.return_request(UserResponsePost, HTTPStatus.BAD_REQUEST)
...
class AdminUser(Resource):
    ...
    def get(self, username):
        authServer.app.logger.info(helpers.log_request_id() + "Admin user '" + username + "' information requested.")
        
        existingUser = authServer.db.adminusers.find_one({"username": username})
    ...
    def put(self, username):
        authServer.app.logger.info(helpers.log_request_id() + "Admin user '" + username + "' update requested.")
        ...
        if (existingUser is not None):
            if (existingUser["account_closed"] == False):            
                ...
                authServer.db.adminusers.update_one({"username": username}, {'$set': userToUpdate})
    ...
    def patch(self, username):
        authServer.app.logger.info(helpers.log_request_id() + "Password modification for admin user '" + username + "' requested.")
        try:
            parser = reqparse.RequestParser()
            parser.add_argument("op", type=helpers.non_empty_string, required=True, nullable=False, choices=['replace'])
            parser.add_argument("path", type=helpers.non_empty_string, required=True , nullable=False, choices=['/password'])
        ...
        if (existingUser is not None):
            if (existingUser["account_closed"] == False):            
                ...
                authServer.db.adminusers.update_one({"username": username}, {'$set': existingUser})

                userResponsePatch = {
                    "code": 0,
                    "message": "Admin user '" + username + "' password updated.",
                    "data:": None
                }
    ...
    def delete(self, username):
        authServer.app.logger.info(helpers.log_request_id() + "Admin user '" + username + "' close account requested.")
 
        existingUser = authServer.db.adminusers.find_one({"username": username})
        if (existingUser is not None):
            if (existingUser["account_closed"] == False):            
            
                authServer.db.adminusers.update_one({"username": username}, {'$set': {'account_closed': True, 'date_updated': datetime.utcnow().isoformat()}})
                authServer.db.sessions.delete_many({"username": username})

                UserResponseDelete = {
                    "code": 0,
                    "message": "Admin user '" + username + "' marked as closed account.",
                    "data": None
                }
            ...
            UserResponseDelete = {
                "code": -1,
                "message": "Admin user '" + username + "' account is already closed.",
                "data": None
            }
...
class AdminUserSessions(Resource):
    ...
    @helpers.log_reqId
    def get(self, username):
        authServer.app.logger.info(helpers.log_request_id() + "Admin user '" + username + "' sessions requested.")
